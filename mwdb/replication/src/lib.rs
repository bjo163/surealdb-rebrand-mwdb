...
impl From<SignedBytes> for WireSignedEnvelope {
    fn from(value: SignedBytes) -> Self {
        Self { protocol: value.protocol, peer_id: value.peer_id, public_key: value.public_key.to_vec(), capabilities: Vec::new(), payload: value.payload, signature: value.signature.to_vec() }
    }
}
...
mod tests {
    ...
    fn sender_nonce_survives_restart() {
        let (a_dir, b_dir, _a, mut b) = sessions(7, 8);
        let f1 = {
            let mut a = ReplicationSession::open(a_dir.path(), "node-a", [7; 32], [7; 32]).unwrap();
            a.store_mut().set("doc:1", serde_json::json!(1)).unwrap();
            a.make_frame(None).unwrap()
        };
        let mut a_key = ReplicationSession::open(a_dir.path(), "node-a", [7; 32], [7; 32]).unwrap();
