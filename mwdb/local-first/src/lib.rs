use std::{collections::HashSet, fs::{File, OpenOptions}, io::{BufRead, BufReader, Write}, path::{Path, PathBuf}};
...
pub struct ChangeQueue {
    ...
    status: std::collections::HashMap<Uuid, ChangeStatus>,
}
...
impl ChangeQueue {
    ...
    pub fn status(&self, change_id: Uuid) -> Option<&ChangeStatus> { self.status.get(&change_id) }

    pub fn all(&self) -> &[ChangeEnvelope] { &self.changes }
    ...
    pub fn known_ids(&self) -> HashSet<Uuid> { self.status.keys().copied().collect() }
}
...
impl LocalFirstStore {
    pub fn open(dir: impl AsRef<Path>, actor_id: impl Into<String>) -> Result<Self, LocalFirstError> {
        ...
        Ok(Self { queue, state_path, state, actor_id: actor_id.into() })
    }

    pub fn get(&self, key: &str) -> Option<&serde_json::Value> { self.state.get(key) }
    ...
    pub fn set(&mut self, key: impl Into<String>, value: serde_json::Value) -> Result<Uuid, LocalFirstError> {
        ...
        let change = ChangeEnvelope {
            ...
            operation: "set".into(),
            payload: value.clone(),
            created_at_ms: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis(),
        };
    ...
    pub fn queue(&self) -> &ChangeQueue { &self.queue }

    pub fn queue_mut(&mut self) -> &mut ChangeQueue { &mut self.queue }
...
mod tests {
    ...
    fn acknowledgement_is_idempotent() {
        let dir = tempdir().unwrap();
        let mut store = LocalFirstStore::open(dir.path(), "device-a").unwrap();
        let id = store.set("k", serde_json::json!(1)).unwrap();
        store.queue_mut().set_status(id, ChangeStatus::Acknowledged).unwrap();
        assert!(store.queue().pending().is_empty());
        assert!(store.queue_mut().set_status(id, ChangeStatus::Acknowledged).is_ok());
        assert!(store.queue().pending().is_empty());
    }
    ...
    fn duplicate_change_id_is_not_reenqueued() {
        ...
        let id = Uuid::new_v4();
        let c = ChangeEnvelope { change_id: id, actor_id: "a".into(), object_id: "o".into(), operation: "set".into(), payload: serde_json::json!(true), created_at_ms: 1 };
